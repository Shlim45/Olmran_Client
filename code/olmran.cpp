                         ...
                         int32 AlignX = 0, int32 AlignY = 0)
{
    ...
    // TODO(jon): SourceRow needs to be changed based on clipping.
    ...
    for (int32 Y = MinY; Y < MaxY; Y++)
    {
        ...
        for (int32 X = MinX; X < MaxX; X++)
        {
            ...
            // TODO(jon): This is not premultiplied alpha.
            real32 R = (1.0f-A)*DR + A*SR;
            real32 G = (1.0f-A)*DG + A*SG;
            real32 B = (1.0f-A)*DB + A*SB;
            
            *Dest = (uint32)(((TruncateReal32ToInt32(R) << 16) |
                              (TruncateReal32ToInt32(G) << 8) |
                              (TruncateReal32ToInt32(B) << 0)));
...
internal loaded_bitmap DEBUGLoadBMP(thread_context *Thread, debug_platform_read_entire_file *ReadEntireFile, char *FileName)
{
    ...
    if (ReadResult.ContentsSize != 0)
    {
        ...
        // NOTE(jon): Byte order in memory is determined by the Header itself,
...
extern "C" GAME_UPDATE_AND_RENDER(GameUpdateAndRender)
{
    ...
    if (!Memory->IsInitialized)
    {
        ...
        GameState->PlayerP.AbsTileZ = 0;    // NOTE(jon): Unnecessary since initialized to 0, but Jon added
        ...
        {
            // TODO(jon): Random number generator!
    ...
    for (int ControllerIndex = 0; ControllerIndex < ArrayCount(Input->Controllers); ControllerIndex++)
    {
        ...
        else
        {
            ...
            // TODO(jon): Diagnols will be faster!  Fix once we have vectors :)
